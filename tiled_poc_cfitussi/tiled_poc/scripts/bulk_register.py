...
def prepare_node_data(ham_df, art_df, max_hamiltonians):
    ...
    for _, ham_row in ham_subset.iterrows():
        ...
        metadata = {
            "huid": huid,
            "F2_dd": float(ham_row["F2_dd"]),
            "F2_dp": float(ham_row["F2_dp"]),
            "F4_dd": float(ham_row["F4_dd"]),
            "G1_dp": float(ham_row["G1_dp"]),
            "G3_dp": float(ham_row["G3_dp"]),
            "Gam_c": float(ham_row["Gam_c"]),
            "sigma": float(ham_row["sigma"]),
            "soc_c": float(ham_row["soc_c"]),
            "soc_v_i": float(ham_row["soc_v_i"]),
            "soc_v_n": float(ham_row["soc_v_n"]),
            "tenDq": float(ham_row["tenDq"]),
            "xoffset": float(ham_row["xoffset"]),
        }
        ...
        if artifacts is not None:
            for _, art_row in artifacts.iterrows():
                ...
                #slice_idx = art_row["index_in_file"] #MODIFICATION FOR SLICER

                # Get shape
                
                shape_default = (default_shapes or {}).get(artifact_type, [1])

                
                
                if artifact_type == "rixs":
                    data_shape = tuple(shape_default)#[1:]   #MODIF

                elif artifact_type == "xps":
                    data_shape = tuple(shape_default)#[1:] #(5000) MODIF

                elif artifact_type == "rixs_energy_in":
                    data_shape = (shape_default[0],) #(40)

                elif artifact_type == "rixs_energy_loss":
                    data_shape = (shape_default[0],) #(151)

                elif artifact_type == "xps_energy":
                    data_shape = (shape_default[0],)
                else:
                ...
                if artifact_type == "rixs":
                    art_metadata["axis_x"] = "/eloss_RIXS"
                    art_metadata["axis_y"] = "/ein_RIXS"                
                elif artifact_type == "xps":
                    art_metadata["axis_x"] = "/w_xps"
                ...
                art_data_sources.append({
                    ...
                    #MODIFICATIONS
                    #"mimetype": "application/x-sliced-hdf5",  
                    #"parameters": json.dumps({
                    #"dataset": dataset_paths.get(artifact_type),
        ...
        for ds in art_data_sources:
            ...
            result = conn.execute(
                ...
                """),
                {
                    ...
                    #"mimetype": ds["mimetype"],#MODIFICATION FOR SLICER
                    #"parameters": ds["parameters"], #MODIFICATION FOR SLICER
